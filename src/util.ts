import { BlankNode, DataFactory, DefaultGraph, Literal, NamedNode, Prefixes, Quad, Store, Term as N3Term } from 'n3'
import {
    DATA_GRAPH,
    OWL_PREDICATE_IMPORTS,
    PREFIX_DCTERMS,
    PREFIX_FOAF,
    PREFIX_RDF,
    PREFIX_RDFS,
...
function findVisibleClassInstanceGraphs(
    ...
    const valuesGraph = context.config.valuesGraphId ?? DataFactory.defaultGraph()
    graphs.set(valuesGraph.id, valuesGraph)

    for (const owlImport of context.owlImports) {
        addImportGraph(owlImport, store, graphs)
    }
    if (context.parent) {
        findVisibleClassInstanceGraphs(context.parent, store, graphs)
    }
    return [...graphs.values()]
}

function addImportGraph(importGraph: NamedNode, store: Store, graphs: Map<string, NamedNode | DefaultGraph>) {
    if (graphs.has(importGraph.id)) {
        return
    }
    graphs.set(importGraph.id, importGraph)
    for (const recursiveImport of store.getObjects(null, OWL_PREDICATE_IMPORTS, importGraph)) {
        if (recursiveImport.termType === 'NamedNode') {
            addImportGraph(recursiveImport as NamedNode, store, graphs)
        }
    }
}
...
export function findInstancesOf(clazz: NamedNode, template: ShaclPropertyTemplate): InputListEntry[] {
    ...
    if (template.in) {
        ...
        const visibleGraphs = findVisibleClassInstanceGraphs(template, template.config.store)
        const instances = getSubjectsFromGraphs(template.config.store, RDF_PREDICATE_TYPE, clazz, visibleGraphs)
        ...
        for (const instance of instances) {
            for (const parent of getObjectsFromGraphs(template.config.store, instance, SKOS_PREDICATE_BROADER, visibleGraphs)) {
                if (nodes.has(parent.id)) {
                    childToParent.set(instance.id, parent.id)
                }
            }
            for (const child of getObjectsFromGraphs(template.config.store, instance, SKOS_PREDICATE_NARROWER, visibleGraphs)) {
                if (nodes.has(child.id)) {
                    childToParent.set(child.id, instance.id)
                }
            }
            for (const parent of getObjectsFromGraphs(template.config.store, instance, RDFS_PREDICATE_SUBCLASS_OF, visibleGraphs)) {
                if (nodes.has(parent.id)) {
                    childToParent.set(instance.id, parent.id)
                }
            }
        ...
        for (const subClass of getSubjectsFromGraphs(template.config.store, RDFS_PREDICATE_SUBCLASS_OF, clazz, visibleGraphs)) {
